...
# Python modules
export PYTHONPATH=${PYTHONPATH:-''} # default to empty
for python_mod_dir in $(ls -d /var/vcap/packages/*/lib/python*/site-packages 2>/dev/null)
do
...
for package_bin_dir in $(ls -d /var/vcap/packages/*/lib/python*/lib-dynload 2>/dev/null)
do
